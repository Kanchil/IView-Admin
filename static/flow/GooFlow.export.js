...
GooFlow.prototype.exportDiagram = function(fileName) {
    // 0.添加临时元素
  const width = this.$workArea.width();
  const height = this.$workArea.height();
  $('body').append('<div id="demo_export" style="position:absolute;top:0;left:0;z-index:-1;width:0px;height:0px;overflow:hidden">'
    ...
    // 1.先COPY节点和区块的内容
  const inner = $('#demo').find('.GooFlow_work_inner');
  const divCanvas = $('#demo_export').children('div:eq(0)');
    // 复制节点的内容
  ...
  html2canvas(divCanvas[0], {
    ...
    onrendered(canvas) {
      ...
      context.save();
      const strSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="1160" height="507">'
