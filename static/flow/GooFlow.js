...
function getDateFromFormat(dateString, formatString) {
    const regDate = /\d+/g;
    const regFormat = /[YyMmdHhSs]+/g;
    const dateMatches = dateString.match(regDate);
    const formatmatches = formatString.match(regFormat);
    const date = new Date();
...
function parseCurrency(currentString) {
    const regParser = /[\d\.]+/g;
    const matches = currentString.match(regParser);
    let result = '';
    const dot = false;
        ...
        this.$group.on("mousedown",{inthis:this},function(e){//绑定RESIZE功能以及移动功能
            ...
            document.onmousemove=function(e){
                ...
                if(cursor!="move"){
                    ...
                    //减去多余偏移
                    This.$ghost.css({left:X -310 +"px",top:Y -175 +"px"});
                }
                isMove=true;
            }
            document.onmouseup=function(e){
    ...
    initWorkForNode:function(){
        ...
        this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
            ...
            This.$textArea.val(oldTxt).css({display:"block",width:$(this).width()+26,height:$(this).height()+6,
                left:t.left+26+This.$nodeData[id].left*This.$scale-This.$workArea[0].parentNode.scrollLeft -320,
                top:t.top+2+This.$nodeData[id].top*This.$scale-This.$workArea[0].parentNode.scrollTop -185})
                .data("id",This.$focus).focus();
            This.$workArea.parent().one("mousedown",function(e){
                if(e.button==2)return false;
                This.setName(This.$textArea.data("id"),This.$textArea.val(),"node");
...
///////////以下为有关画线的方法
    ...
    addLine:function(id,json){
        ...
        this.$lineData[id].id=id;
        this.$lineData[id].selectConditionNodeList = [];
        if(json.selectConditionNodeList){
            this.$lineData[id].selectConditionNodeList= json.selectConditionNodeList;
            console.log(id,json.selectConditionNodeList);
        }
        this.$lineData[id].bpmTypeId=json.bpmTypeId;
